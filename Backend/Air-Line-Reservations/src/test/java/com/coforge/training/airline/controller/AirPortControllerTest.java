...
// assertNotNull(res); //(assertnot null)=a passed parameter must not be null
// for
...
// assertNotNull(addc.get(0).getAdminemail()); //a passed parameter must not be
// null if it is null then test will be fail
...
// when(service.deleteAirport(airport.get(0).getAirportid())).thenReturn(del);//
// if id is present the return del
// String str=service.deleteAirport(airport.get(0).getAirportid());
// assertNotNull(str); //a passed parameter must not be null if it ia null then
// test case fails
